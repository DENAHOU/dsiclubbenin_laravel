        ...
        <form method="POST" action="{{ route('admin.events.update', $event->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('type_event_id') is-invalid @enderror" 
                    ...
                    @foreach($eventTypes as $eventType)
                        <option value="{{ $eventType->id }}" {{ old('type_event_id', $event->type_event_id) == $eventType->id ? 'selected' : '' }}>
                            {{ $eventType->nom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        @if($event->image)
                            <small class="text-muted d-block mt-1">Image actuelle: {{ $event->image }}</small>
                        @endif
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="url" class="form-control @error('video_url') is-invalid @enderror" 
                               id="video_url" name="video_url" value="{{ old('video_url', $event->video_url) }}" 
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('date') is-invalid @enderror" 
                               id="date" name="date" value="{{ old('date', $event->date ? $event->date->format('Y-m-d') : '') }}" required>
                        @error('date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" 
                    ...
                    <option value="actif" {{ old('status', $event->status) == 'actif' ? 'selected' : '' }}>Actif</option>
                    <option value="inactif" {{ old('status', $event->status) == 'inactif' ? 'selected' : '' }}>Inactif</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
