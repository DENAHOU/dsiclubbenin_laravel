...
<div class="admin-header">
    <h1>Modifier un Programme Thématique</h1>
        ...
        <form method="POST" action="{{ route('admin.programs.update', $program->id) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('titre') is-invalid @enderror" 
                       id="titre" name="titre" value="{{ old('titre', $program->titre) }}" required>
                @error('titre')
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" 
                    ...
                    <option value="actif" {{ old('status', $program->status) == 'actif' ? 'selected' : '' }}>Actif</option>
                    <option value="inactif" {{ old('status', $program->status) == 'inactif' ? 'selected' : '' }}>Inactif</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
