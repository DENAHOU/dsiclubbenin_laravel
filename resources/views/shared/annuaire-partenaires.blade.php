...
@section('title', 'Annuaire des Partenaires')
    ...
    <div class="container-fluid py-4">
                ...
                <div class="row align-items-center">
                    <div class="col-lg-8">
                        <div class="filter-tabs">
                            ...
                            @foreach($stats['types'] as $partnerTypeId => $count)
                                <a href="?type={{ $partnerTypeId }}" class="filter-tab {{ $type == $partnerTypeId ? 'active' : '' }}">
                                    <i class="fas fa-star me-2"></i>{{ $partnerTypes[$partnerTypeId] ?? 'Type ' . $partnerTypeId }}
                                </a>
                            @endforeach
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="search-box">
                            <input type="text" name="search" class="search-input" placeholder="Rechercher un partenaire..." value="{{ $search }}">
        ...
        @elseif($partenaires->count() > 0)
                    ...
                    <div class="partner-card">
                        ...
                        <div class="partner-logo-container">
                            ...
                            @else
                                <div class="partner-logo d-flex align-items-center justify-content-center" style="background: var(--primary-gradient); color: white; font-size: 1.5rem; font-weight: bold;">
                                    {{ substr($partner->company_name, 0, 2) }}
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="partner-info">
                            ...
                            <span class="partner-type type-standard">
                                {{ $partnerTypes[$partner->partner_type_id] ?? 'Standard' }}
                            </span>
                            ...
                            <div class="partner-actions">
                                @if($partner->website_url)
                                    <a href="{{ $partner->website_url }}" target="_blank" class="action-btn btn-primary-custom">
                                        <i class="fas fa-external-link-alt me-2"></i>Site
                                    </a>
                                @endif
                                
                                @if($partner->email)
                                    <button onclick="copyContactInfo('{{ $partner->email }}', '{{ $partner->company_name }}')" class="action-btn btn-secondary-custom">
            ...
            <div class="d-flex justify-content-center mt-4">
                @if($partenaires->currentPage() > 1)
                    <a href="{{ $partenaires->previousPageUrl() }}" class="btn-pagination">
                        <i class="fas fa-chevron-left me-2"></i>Précédent
                    </a>
                @endif
                
                @if($partenaires->hasMorePages())
                    <a href="{{ $partenaires->nextPageUrl() }}" class="btn-pagination">
